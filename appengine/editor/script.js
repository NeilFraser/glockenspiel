...
Music.blocksToCode = function() {
  ...
  code = Music.naturalToAccidental(code);
  var header = '';
 ...
 * E.g. 'play(1/4, C7 + 1, id);' -> 'play(1/4, Db7, id);'
 * @param {string} code Code generated by Blockly.
 * @returns {string} Modified code using accidental variables.
 */
Music.naturalToAccidental = function(code) {
  ...
  for (var i = 0; i < notes.length; i++) {
    ...
    if (down) {
      var r = new RegExp(', ' + origin + ' - 1, ', 'g');
      code = code.replace(r, ', ' + down + ', ');
    }
    if (up) {
      var r = new RegExp(', ' + origin + ' \\+ 1, ', 'g');
      code = code.replace(r, ', ' + up + ', ');
      var r = new RegExp(', 1 \\+ ' + origin + ', ', 'g');
      code = code.replace(r, ', ' + up + ', ');
    }
  }
  return code;
